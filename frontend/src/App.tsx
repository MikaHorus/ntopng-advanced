    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        <header className="topbar">
          <div><span className="eyebrow">CENTRE DE CONTRÔLE</span><h1>Vue réseau</h1></div>
          <div className="top-actions">
            <label className="interface-select">Interface
              <select value={selectedInterface} onChange={(event) => { const id = Number(event.target.value); setSelectedInterface(id); void loadDashboard(id); }}>
                {interfaces.map((item) => <option key={item.interface_id} value={item.interface_id}>{item.name} · ifid {item.interface_id}</option>)}
              </select>
            </label>
            <button className="icon-button" title="Actualiser" onClick={() => void load()}><RefreshCw size={18} className={loading ? "spin" : ""} /></button>
          </div>
        </header>

        <section className="status-strip">
          <div className="live-status"><span className="pulse" /> Connexion ntopng opérationnelle</div>
          <span>Actualisé à l’ouverture · interface {interfaces.find((item) => item.interface_id === selectedInterface)?.name ?? "—"}</span>
        </section>

        {error && <div className="error-banner"><Database size={18} /> {error}<button onClick={() => void load()}>Réessayer</button></div>}
        ...
        <section className="content-grid">
          <div className="panel traffic-panel">
            <div className="panel-heading"><div><span className="eyebrow">CAPTURE EN DIRECT</span><h2>État de l’interface</h2></div><span className="live-badge"><Activity size={14} /> LIVE</span></div>
            <div className="traffic-visual"><div className="traffic-line" /><div className="traffic-line muted" /><div className="traffic-label"><strong>{numberValue(interfaceData.throughput_bps)} <small>bps</small></strong><span>Débit actuel</span></div></div>
            <div className="traffic-foot"><span><i className="dot blue-dot" /> Download <strong>{bytesDownload} octets</strong></span><span><i className="dot orange-dot" /> Upload <strong>{bytesUpload} octets</strong></span></div>
          </div>
          <div className="panel interface-panel">
            <div className="panel-heading"><div><span className="eyebrow">POINT DE CAPTURE</span><h2>{interfaces.find((item) => item.interface_id === selectedInterface)?.name ?? "Interface"}</h2></div><Server size={22} className="muted-icon" /></div>
            <div className="detail-list"><Detail label="Hôtes vus" value={numberValue(interfaceData.num_hosts)} /><Detail label="Flows observés" value={numberValue(interfaceData.num_flows)} /><Detail label="Paquets" value={numberValue(interfaceData.packets)} /><Detail label="Uptime" value={String(interfaceData.uptime ?? "—")} /></div>
          </div>
        </section>
